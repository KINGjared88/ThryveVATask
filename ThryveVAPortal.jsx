...
const ThryveVAPortal = () => {
  ...
  const handleAddTask = (day, text) => {
    if (!text.trim()) return;
    const newTask = { id: Date.now(), text, completed: false, notes: '' };
    setTasksByDay(prev => ({
      ...prev,
  ...
  const handleNoteChange = (day, id, notes) => {
    setTasksByDay(prev => ({
      ...prev,
      [day]: prev[day].map(task =>
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 p-6 md:p-10 lg:p-12 max-w-7xl mx-auto">
      ...
      {/* Reset button (admin only placeholder) */}
          ...
          <div key={day} className="bg-white rounded-2xl shadow-lg p-5 border border-gray-100">
            ...
            <div className="space-y-4">
              {tasksByDay[day].map(task => (
                <div
                  key={task.id}
                  className={`rounded-xl shadow-sm p-4 border border-gray-200 transition duration-200 ease-in-out ${
                    task.completed ? 'bg-blue-50' : 'bg-white hover:shadow-md'
                  }`}
                >
                  ...
                  <textarea
                    ...
                    className="w-full text-sm mt-3 p-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400"
                    rows={2}
                    value={task.notes}
